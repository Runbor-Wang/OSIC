...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, features, tgt, tgt_mask):
        ...
        return self.decode(features, tgt, tgt_mask)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, tgt_mask):
        memory = self.linear_last(memory)
        for layer in self.layers:
...
class OSICNodmseNorefineModel(CaptionModel):
    def make_model(self, src_vocab, tgt_vocab, swin_model_c=96, N=6, d_model=512, d_ff=2048, h=8, dropout=0.1):
        ...
        model = EncoderDecoder(Decoder(DecoderLayer(d_model, c(attn), c(attn), c(ff), dropout), N),
                               lambda x: x, nn.Sequential(Embeddings(d_model, tgt_vocab), c(position)),
    ...
    def __init__(self, opt):
        super(OSICNodmseNorefineModel, self).__init__()
        self.opt = opt
