...
class EncoderDecoder(nn.Module):
    ...
    other models.
    """
    def __init__(self, refine_encoder, decoder, src_embed, tgt_embed, generator):
    ...
    def forward(self, features, tgt, tgt_mask):
        ...
        return self.decode(self.refine_encode(features), tgt, tgt_mask)
...
class FuseEncoder(nn.Module):
    "Define standard linear + softmax generation step."
    def __init__(self, swin_model_c):
        super(FuseEncoder, self).__init__()
        self.swin_model_c = swin_model_c  # 96
    ...
    def se_fuse(self, x):
        y = self.fuse_global(x.transpose(-1, -2))  # [10, 1920, 1]
        y = self.fuse_se_0(y.transpose(-1, -2))  # [10, 1, 240]
        y = self.fuse_se_1(y)  # [10, 1, 1920]
        return x + self.norm_fuse(x * y)

    def forward(self, x_0, x_1, x_2, x_3):
        x_0, x_1, x_2, x_3 = [l(x) for l, x in zip(self.adjust_linears_16, (x_0, x_1, x_2, x_3))]
        x = self.merges(x_0, 2)
        x = self.norms[0](x)
        ...
        x = torch.cat([x_2, x], -1)
        x_3 = self.norms[3](x_3)
        x = torch.cat([x_3, x], -1)

        return self.fuse_linear(self.se_fuse(x))
...
class RefineEncoder(nn.Module):
    ...
    def __init__(self, layer, N):
        ...
        self.linear_last = nn.Linear(1536, 512)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, tgt_mask):
        # print("244 Decoder self.N", self.N)
        for layer in self.layers:
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    d_k = query.size(-1)
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)  # [10, 8, 144, 144]
    if mask is not None:
...
class OSICNoFusionModel(CaptionModel):
    def make_model(self, src_vocab, tgt_vocab, swin_model_c=96, N=6, DeN=6, d_model=512, s_model=144, d_ff=2048, h_p=8, h_c=4, dropout=0.1):
        ...
        model = EncoderDecoder(RefineEncoder(RefineEncoderLayer(d_model, c(attn), c(ff), dropout), N),
    ...
    def __init__(self, opt):
        super(OSICNoFusionModel, self).__init__()
        self.opt = opt
    ...
    def _prepare_input(self, x, seq=None, images_masks=None):
        x = pack_wrapper(lambda x: x, x, images_masks)
        ...
        return x, seq, seq_mask

    def _forward(self, x, seq):
        """
        images: [B*5, 3, 384, 384]
        seq: [B*5, 17]
        seq_masks: [B*5, 17, 17] ==> bool type: subsequent_mask with np.triu()
        """
        x_, seq, seq_masks = self._prepare_input(x, seq)
        out = self.model(x, seq, seq_masks)
    ...
    def _sample_beam(self, x, opt={}):
        beam_size = opt.get('beam_size', 10)
        batch_size = x.size(0)

        x, seq, seq_masks = self._prepare_input(x)
        memory = self.model.refine_encode(x)
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                if t == 0:  # input <bos>
                    it = x.new_zeros([beam_size], dtype=torch.long)
    ...
    def _sample_(self, x, opt={}):
    # def _sample_(self, image, opt={}):
        sample_max = opt.get('sample_max', 1)
        beam_size = opt.get('beam_size', 1)
        temperature = opt.get('temperature', 1.0)
        if beam_size > 1:
            return self._sample_beam(x, opt)

        if sample_max:
            with torch.no_grad():
                seq_, seqLogprobs_ = self._sample_(x, opt)

        batch_size = x.shape[0]

        x, seq, seq_masks = self._prepare_input(x)

        memory = self.model.refine_encode(self.model.fuse_encode(x))
        ys = torch.zeros((batch_size, 1), dtype=torch.long).to(x.device)

        seq = x.new_zeros((batch_size, self.seq_length), dtype=torch.long)
        seqLogprobs = x.new_zeros(batch_size, self.seq_length)

        for i in range(self.seq_length):
            out = self.model.decode(memory, ys, subsequent_mask(ys.size(1)).to(x.device))
    ...
    def _sample(self, x, opt={}):
        ...
        for t in range(self.seq_length + 1):
            if t == 0:  # input <bos>
                it = x.new_zeros(batch_size, dtype=torch.long)
